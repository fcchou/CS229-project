...
svd_1 = SVD(n_components = 200)
X = svd_1.fit_transform(feature2_norm)

sfk = cv.StratifiedShuffleSplit(labels, 40)
#estimators = [('reduction', TruncatedSVD()), 
#              ('learn', SVC(class_weight='auto'))]
#clf = Pipeline(estimators)    

#params = {'reduction__n_components':range(10, 400, 50), 'learn__C':[100, 1000, 10000, 100000]}
#gs = GridSearchCV(
#        clf, params, scoring='f1',
#        n_jobs=4, cv=sfk, verbose=1)
#gs.fit(feature1_norm, labels)
#with open('reduction_svd_brfSVC_cp.out','w') as f:
#    for grid in gs.grid_scores_:
#        print >> f,  grid.parameters, grid.mean_validation_score
