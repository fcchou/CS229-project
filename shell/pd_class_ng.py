...
for name in glob('../learning/npz_data/*.npz'):
    npz = np.load(name)
    ...
    pd_class_arr = np.zeros([n_move,2])
    for i_note in range(n_move):
        if combined_arr[i_note,0] and combined_arr[i_note+1,0] != 0:
            p_class = combined_arr[i_note+1,0] - combined_arr[i_note,0]
            d_class = combined_arr[i_note+1,1]/combined_arr[i_note,1]
            pd_class_arr[i_note] = np.array([p_class, d_class])
    # pd_class_arr = np.delete(pd_class_arr, 0)
    # pd_class_arr = np.delete(pd_class_arr, -1)

    outdir = './pd_class_ng_arrs/'
